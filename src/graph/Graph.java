...
public class Graph {
	...
	/**
	 * Creates a new <code>Graph</code> instance. These are random instances
	 * with n nodes and an edge probability of 0.5
	 */
			...
			if (colouring[current] < k - 1) {
				...
				if (current == size() - 1) {
					if (partial_cost[current] < best_colouring.cost()) {
						best_colouring = new Colouring(colouring, partial_cost[current], k);
					}
				} else {
					current++;
				}
	...
	public void show(Colouring colouring) {
		Color[] colour = { Color.red, Color.yellow, Color.blue, Color.green, Color.black, Color.orange, Color.magenta,
				Color.cyan, Color.gray, Color.pink };
		System.out.println("Number of colour conflicts = " + colouring.cost());
